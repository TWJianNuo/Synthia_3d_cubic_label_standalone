...
function [loss, num_on_cubic, ratio_off_obj, max_diff] = metric_pos_and_inv(cuboid, objs, depth_cluster)
    ...
    max_diff_rec = zeros(length(objs),1); num_sf_inf = -inf;
    for i = 1 : length(objs)
        ...
        val1 = max((abs(depth_cluster.depth_maps{depth_map_ind}(linear_ind_pos(ica)) - depth_pos(selector_pos))) ./ depth_pos(selector_pos));
        val2 = max((abs(depth_inv(select_p1) - d1((x11(:,1) >= 0) & (plane_type_rec == 1)))) ./ depth_inv(select_p1));
        val3 = max((abs(depth_inv(select_p2) - d2((x22(:,1) >= 0) & (plane_type_rec == 2)))) ./ depth_inv(select_p2));
        try 
            max_diff_rec(i) = max([val1,val2,val3]);
        catch
            max_diff_rec(i) = 0;
        end
