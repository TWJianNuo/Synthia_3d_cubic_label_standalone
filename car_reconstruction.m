help_info = helper();
estimate_affine_matrix(helper);
%{
for i = 1 : size(help_info, 1)
    ...
    % folder_path = make_dir(save_path, base_path); 
    % file_ids = create_txt(folder_path);
    for j = 1 : n
        frame = j;
        ...
        [affine_matrx, mean_error] = estimate_ground_plane(intrinsic_params, extrinsic_params, depth, label);
        objs = seg_image(depth, label, instance, extrinsic_params, intrinsic_params, affine_matrx);
        objs = get_init_guess(objs);
        img = render_image(objs, rgb);
        % save_work(file_ids, objs, frame, img, rgb, folder_path, intrinsic_params, extrinsic_params, affine_matrx);
    end
end
%}
